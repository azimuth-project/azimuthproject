...
public:
    RandBufferUniformU32(int sCapacity) { setup(sCapacity); } 
    ~RandBufferUniformU32();
    void setup(int sCapacity);
    void fillBuffer();
...
struct PolarRVCreator {
    static void transform(RandBufferUniformU32* uniformGenerator,LL_SIMD &U,LL_SIMD &V,LL_SIMD &R_SQR) {
        ...
        static const LL_SIMD scaling=_mm_set1_ps(float(RandBufferUniformU32::MAX_RAND));
        do{
...
class RVSIMDStream : public RVSIMDStreamBase { public:
...
//typedef RVSIMDStream<> CauchySIMDStream;
//typedef RVSIMDStream<> CauchySIMDStream;
